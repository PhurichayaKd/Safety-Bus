...
CREATE TABLE public.driver_bus (
  ...
  trip_phase text DEFAULT 'go'::text CHECK (trip_phase = ANY (ARRAY['go'::text, 'return'::text, 'unknown'::text])),
...
CREATE TABLE public.rfid_scan_logs (
  ...
  latitude numeric,
  longitude numeric,
  location_type character varying DEFAULT 'unknown'::text CHECK (location_type::text = ANY (ARRAY['go'::text, 'return'::text, 'unknown'::text])),
...
CREATE TABLE public.student_boarding_status (
  ...
  trip_phase character varying NOT NULL DEFAULT 'go'::character varying CHECK (trip_phase::text = ANY (ARRAY['go'::text, 'return'::text, 'unknown'::text])),
