-- Supabase Function สำหรับบันทึกการสแกน RFID และส่งแจ้งเตือน LINE (เข้ากันได้กับตารางที่มีอยู่)
  ...
  IF v_line_user_id IS NOT NULL THEN
    BEGIN
      ...
      IF (line_result->>'success')::boolean THEN
        ...
        -- บันทึก log สำเร็จ (ใช้โครงสร้างตารางที่มีอยู่จริง)
      ...
      ELSE
        ...
        -- บันทึก log ล้มเหลว (ใช้โครงสร้างตารางที่มีอยู่จริง)
        INSERT INTO notification_logs (
          notification_type, 
          recipient_id, 
          message, 
          ...
          notification_message,
          'failed',
          json_build_object('error', line_result->>'error')
        );
      END IF;
    EXCEPTION
      WHEN OTHERS THEN
        ...
        -- บันทึก log error (ใช้โครงสร้างตารางที่มีอยู่จริง)
        INSERT INTO notification_logs (
          notification_type, 
          recipient_id, 
          message, 
          ...
          notification_message,
          'error',
          json_build_object('exception', SQLERRM, 'sqlstate', SQLSTATE)
        );
    END;
  END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    -- บันทึก error log (ใช้โครงสร้างตารางที่มีอยู่จริง)
    INSERT INTO notification_logs (
      notification_type, 
      recipient_id, 
      message, 
      ...
      'error',
      json_build_object('sqlstate', SQLSTATE, 'error', SQLERRM, 'rfid_code', p_rfid_code)
    );
    ...
    RETURN json_build_object(
      ...
      'sqlstate', SQLSTATE,
      'rfid_code', p_rfid_code,
      'debug_info', 'Function updated with compatible table structure'
    );
END;
